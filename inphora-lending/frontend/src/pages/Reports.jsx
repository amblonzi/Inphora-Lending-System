...
const Reports = () => {
    ...
    const [pnLData, setPnLData] = useState({ total_income: 0, total_expenses: 0, net_profit: 0, expense_breakdown: [] });
    const [parData, setParData] = useState({ par_distribution: {}, par_ratios: {}, total_active_portfolio: 0 });
    const [healthData, setHealthData] = useState({ product_performance: [] });
    ...
    const fetchData = async () => {
        try {
            ...
            setPnLData(pnlRes || { total_income: 0, total_expenses: 0, net_profit: 0, expense_breakdown: [] });
            setParData(parRes || { par_distribution: {}, par_ratios: {}, total_active_portfolio: 0 });
            setHealthData(healthRes || { product_performance: [] });
            setTrendsData(Array.isArray(trendsRes) ? trendsRes : []);
        } catch (error) {
    ...
    const dist = parData?.par_distribution || {};
    const parChartData = [
        { name: 'Current', value: dist.current || 0, color: '#10b981' },
        { name: 'PAR 30', value: dist.par_30 || 0, color: '#f59e0b' },
        { name: 'PAR 60', value: dist.par_60 || 0, color: '#f97316' },
        { name: 'PAR 90', value: dist.par_90 || 0, color: '#ef4444' },
        { name: 'PAR 90+', value: dist.par_90plus || 0, color: '#991b1b' }
    ].filter(d => d.value > 0);
    // safe guard: make sure we always pass an array to chart components
    const parChartDataSafe = Array.isArray(parChartData) ? parChartData : [];
    ...
    // Expense export data
    const expenseExportData = Array.isArray(pnLData?.expense_breakdown) ? pnLData.expense_breakdown.map(e => ({
        "Category": e.category,
                    ...
                    <div className="h-[400px] flex flex-col md:flex-row items-center gap-10">
                        <div className="flex-1 w-full h-full relative">
                            <ResponsiveContainer width="100%" height="100%" minWidth={0} minHeight={0}>
                                <PieChart>
                                    ...
                                    <Pie
                                        ...
                                        stroke="none"
                                    >
                                        {parChartDataSafe.map((entry, index) => (
                            ...
                            <div className="absolute inset-0 flex flex-col items-center justify-center pointer-events-none">
                                ...
                                <span className="text-xl font-black text-gray-900 dark:text-white">KES {((parData.total_active_portfolio || 0) / 1000).toFixed(1)}k</span>
                            </div>
                        </div>
                    ...
                    <div className="max-h-[500px] overflow-y-auto selection:bg-tytaj-500/30">
                        <table className="w-full">
                            ...
                            <tbody className="divide-y divide-gray-100 dark:divide-white/5">
                                {Array.isArray(pnLData.expense_breakdown) && pnLData.expense_breakdown.map((e, idx) => (
                                ...
                                {(!Array.isArray(pnLData.expense_breakdown) || pnLData.expense_breakdown.length === 0) && (
