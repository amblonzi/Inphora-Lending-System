...
const Clients = () => {
  ...
  console.log('DEBUG: Clients State Initial:', clients);
  ...
  const fetchClients = async () => {
    try {
      ...
      // use correct service helper; previous code accidentally referenced api.client
      // if we have any filters, pass them along; clients.list now supports params
  ...
  const exportData = (Array.isArray(clients) ? clients : []).map(client => ({
            ...
            <div className="relative flex-1 sm:flex-none min-w-[180px]">
              ...
              >
                ...
                {(Array.isArray(branches) ? branches : []).map(b => (
                  <option key={b.id} value={b.id} className="dark:bg-[#0a0a0f]">{b.name.toUpperCase()}</option>
                ))}
              </select>
            ...
            <div className="relative flex-1 sm:flex-none min-w-[200px]">
              ...
              >
                ...
                {(Array.isArray(groups) ? groups : []).map(g => (
                  <option key={g.id} value={g.id} className="dark:bg-[#0a0a0f]">{g.name.toUpperCase()}</option>
                ))}
              </select>
              <div className="pointer-events-none absolute inset-y-0 right-0 flex items-center px-3 text-gray-400">
                <Filter size={14} />
          ...
          <table className="w-full">
            ...
            <tbody className="divide-y divide-gray-100 dark:divide-white/5">
              {loading && clients.length === 0 ? (
                ...
                (() => { console.log('DEBUG: Mapping Clients:', clients); return null; })(),
                (Array.isArray(clients) ? clients : []).map((client) => {
                  const branchName = (Array.isArray(branches) ? branches : []).find(b => b.id === client.branch_id)?.name || '-';
                  const groupName = (Array.isArray(groups) ? groups : []).find(g => g.id === client.customer_group_id)?.name || '-';
                  ...
                  return (
                    <tr key={client.id} className="hover:bg-gray-50/50 dark:hover:bg-white/5 transition-all group">
                      ...
                      <td className="px-8 py-5">
                        <span className={`inline-flex items-center gap-1.5 px-3 py-1 rounded-xl text-[10px] font-black uppercase tracking-widest ${client.status === 'active'
