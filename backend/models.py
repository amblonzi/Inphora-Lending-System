...
class Repayment(Base):
    ...
    # M-Pesa specific
    # M-Pesa specific - Temporarily disabled due to schema mismatch
...
class Expense(Base):
    ...
    amount = Column(Float)
    category_id = Column(Integer, ForeignKey("expense_categories.id"), nullable=True) # Soft link or hard FK
    category = Column(String(100)) # Keep existing string for simple migration or fallback
    date = Column(Date)
