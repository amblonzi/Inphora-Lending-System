...
try:
    ...
    for row in data:
        ...
        # Map by name and phone placeholder unique ID matching
        ...
        # compute end date mock
        if db_dur_unit == "months":
            e_date = d_obj + relativedelta(months=1)
        elif db_dur_unit == "weeks":
            e_date = d_obj + relativedelta(weeks=4)
        else:
            e_date = d_obj + relativedelta(days=30)
        ...
        l = Loan(
            ...
            amount=amount,
            interest_rate=0,
            duration_months=1,  # Based on the data showing duration is generic
            start_date=d_obj,
            end_date=e_date,
            ...
            status=db_status,
            total_cost_of_credit=repayable - amount,
        ...
        if repaid > 0:
            ...
            db.add(rep)
            # If repaid >= repayable, it's completed
