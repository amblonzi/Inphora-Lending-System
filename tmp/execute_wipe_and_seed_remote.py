...
# Write local temporary script
with open("tmp/seed_clients_fresh.py", "w") as f:
    f.write(python_script)

# Upload to remote /opt/inphora/
print("Uploading script to remote...")
sftp = ssh.open_sftp()
sftp.put("tmp/seed_clients_fresh.py", "/opt/inphora/seed_clients_fresh.py")
sftp.close()

# Execute on remote
print("Executing wipe and reload on remote...")
commands = [
    "docker cp /opt/inphora/seed_clients_fresh.py backend_tytahj:/app/seed_clients_fresh.py",
    "docker exec backend_tytahj python seed_clients_fresh.py"
]
