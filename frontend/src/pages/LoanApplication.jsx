    ...
    <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
      ...
      <div className="grid grid-cols-2 gap-6">
        <div className="space-y-2">
          <label className="text-[10px] font-black text-gray-400 dark:text-gray-500 uppercase tracking-widest ml-1">
            Duration ({products.find(p => p.id == formData.product_id)?.duration_unit?.toUpperCase() || 'MONTHS'})
          </label>
          <input
            type="number"
      ...
      {formData.product_id && products.find(p => p.id == formData.product_id) && (
        (() => {
          ...
          const procFee = (product.processing_fee_fixed || 0) + ((product.processing_fee_percent || 0) / 100 * (parseFloat(formData.amount) || 0));
          const totalFees = regFee + procFee + (product.insurance_fee || 0) + (product.tracking_fee || 0) + (product.valuation_fee || 0);

          return (
            <div className="p-6 bg-gray-100 dark:bg-white/5 rounded-3xl border border-gray-200 dark:border-white/5 space-y-3">
              <h4 className="text-[10px] font-black text-gray-500 uppercase tracking-widest border-b border-gray-200 dark:border-white/10 pb-2 mb-2">Fee Summary</h4>
              <div className="flex justify-between">
                <span className="text-xs font-bold text-gray-500">Registration Fee</span>
                <span className="text-xs font-bold dark:text-white">KES {regFee.toLocaleString()}</span>
              </div>
              <div className="flex justify-between">
                <span className="text-xs font-bold text-gray-500">Processing Fee</span>
                <span className="text-xs font-bold dark:text-white">KES {procFee.toLocaleString()}</span>
              </div>
              {(product.insurance_fee > 0) && (
                <div className="flex justify-between">
                  <span className="text-xs font-bold text-gray-500">Insurance Fee</span>
                  <span className="text-xs font-bold dark:text-white">KES {product.insurance_fee.toLocaleString()}</span>
                </div>
              )}
              <div className="flex justify-between pt-2 border-t border-gray-200 dark:border-white/10 mt-2">
                <span className="text-xs font-black text-gray-900 dark:text-white uppercase">Total Fees</span>
                <span className="text-xs font-black text-tytaj-500">KES {totalFees.toLocaleString()}</span>
              </div>
            </div>
          );
        })()
