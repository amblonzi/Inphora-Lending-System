...
const DisbursementModal = ({ loan, client, onClose, onSuccess, api }) => {
    ...
    const handleDisburse = async () => {
        ...
        try {
            ...
            if (method === 'mpesa') {
                response = await api.disbursements.mpesa(loan.id, phone);
                toast.success(`M-Pesa disbursement initiated: ${response.message || 'Processing'}`);
            } else if (method === 'bank') {
                response = await api.disbursements.bank(loan.id, bankReference);
                toast.success(`Bank transfer recorded: ${bankReference}`);
            } else {
                response = await api.disbursements.manual(loan.id, notes);
                        ...
                        <div className="space-y-4">
                            ...
                            <div className="grid grid-cols-3 gap-3">
                                <button
                                    ...
                                    className={`p-4 border-2 rounded-2xl flex flex-col items-center gap-2 transition-all group ${method === 'mpesa' ? 'border-tytaj-500 bg-tytaj-500/10 text-tytaj-600 dark:text-tytaj-400' : 'border-gray-100 dark:border-white/5 text-gray-400'
                                        }`}
                                >
