...
const ClientModal = ({ client, onClose, onSave }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // Avoid sending empty next_of_kin if not filled
      if (!formData.next_of_kin.name && !formData.next_of_kin.phone) {
        clientPayload.next_of_kin = null;
      }
          ...
          <div className="flex gap-2 p-2 bg-gray-100/50 dark:bg-black/20 border-b border-gray-100 dark:border-white/5 scroll-x-auto no-scrollbar">
            {(Array.isArray(tabs) ? tabs : []).map(tab => (
              <button
                ...
                className={`flex items-center gap-2.5 px-6 py-3 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all duration-300 whitespace-nowrap ${activeTab === tab.id
                ...
                {activeTab === 'kyc' && (
                  ...
                  >
                    ...
                    {formData.kyc_documents?.length > 0 && (
                      <div className="space-y-3">
                        ...
                        <div className="grid grid-cols-1 gap-3">
                          {Array.isArray(formData.kyc_documents) && formData.kyc_documents.map((doc, idx) => (
                    ...
                    <div className="space-y-4">
                      ...
                      {Array.isArray(kycFiles) && kycFiles.map((kyc, idx) => (
